...
浏览器打开 `http://127.0.0.1:8765/`。页面提供输入、Intent IR、仲裁与策略预览、静态拓扑和指标四栏；拓扑会显示四辆业务车辆、RSU、双路径和 Edge 节点，并在策略预览或验证完成后高亮所选路径。验证指标优先以卡片展示，完整基线/策略后对比表可展开查看；所有策略均需点击“确认下发并验证”才会执行。
